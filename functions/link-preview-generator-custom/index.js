...
const getCanonicalUrl = async (page, uri) => {
  const canonicalUrl = await page.evaluate(() => {
    const ogUrlMeta = document.querySelector('meta[property="og:url"]');
    const twitterUrlMetaName = document.querySelector('meta[name="twitter:url"]');
    const twitterUrlMetaProperty = document.querySelector('meta[property="twitter:url"]');
    const canonicalLink = document.querySelector("link[rel=canonical]");
    const alternateUrl = document.querySelector('link[rel="alternate"][hreflang="x-default"]');
    ...
    if (twitterUrlMetaProperty != null && twitterUrlMetaProperty.content.length > 0) {
...
module.exports = async (
  ...
  try {
    ...
    console.log("Generated link preview for: " + uri);
  ...
  const metascraped = await metascrape(uri);
  const mapMetascraper = { "img": "image", "favicon": "logo", "canonicalUrl": "url", "clearbit": "clearbit", "description": "description" };
  for (const [key, value] of Object.entries(mapMetascraper)) {
    if (!obj[key] && metascraped[value]) {
      obj[key] = metascraped[value];
    }
  }
